        ...
        {taskPane.isEditMode ? (
          <>
            <TextField.Root className="flex items-center">
              ...
              <TextField.Slot>
                <IconButton
                  className=" !m-1 !bg-gray-200 hover:!bg-neutral-600"
                  onClick={saveTask}
                >
                  <FaCheck></FaCheck>
                </IconButton>
          ...
          <>
            <div className="flex items-center justify-between">
              <h1 className="text-md tracking-wider font-bold capitalize">
                {taskPane.title}
              </h1>
              <div className="flex items-center">
                <Button
                  onClick={() => dialogRef.current.click()}
                  className="!cursor-pointer !bg-transparent !text-black hover:!bg-gray-300 "
                >
                  <MdEdit />
                </Button>
                <DropdownMenu.Root>
                  <DropdownMenu.Trigger>
                    <IconButton className=" !m-1 !bg-transparent hover:!bg-neutral-300 !text-xl !text-black">
                      <IoIosMore />
                    </IconButton>
                  </DropdownMenu.Trigger>
                  <DropdownMenu.Content className="min-w-[10rem]">
                    <DropdownMenu.Item
                      ...
                      className=" !m-1  hover:!bg-neutral-600 hover:!text-white !text-black"
                    >
                      Edit Title
                    </DropdownMenu.Item>
                    <DropdownMenu.Separator />
                    <DropdownMenu.Item
                      onClick={deleteTaskPane}
                      className=" !m-1  hover:!bg-red-500 hover:!text-white !text-black"
                    >
                      Delete
                    </DropdownMenu.Item>
            ...
            <div className="flex justify-end">
              <IconButton
                onClick={addSubTask}
                className=" !m-1 !bg-gray-400 hover:!bg-neutral-600 !w-fit !px-4 gap-2"
              >
                <IoAdd />
                Add Subtask
              </IconButton>
        ...
        <Dialog.Content>
          <Dialog.Title className="flex items-center space-x-4">
            ...
            <DropdownMenu.Root>
              ...
              <DropdownMenu.Content>
                <DropdownMenu.Item
                  className="!bg-none hover:bg-gray-50"
                  onClick={editTaskTitle}
                >
                  Edit
                </DropdownMenu.Item>
          ...
          <div>
            ...
            {taskPaneDesc.isEditMode ? (
              <TextArea
                onChange={(e) =>
                  setTaskPaneDesc({ ...taskPaneDesc, value: e.target.value })
                }
                value={taskPaneDesc.value}
              ></TextArea>
            ) : (
              <div
                className="hover:bg-gray-50 py-3 px-2 cursor-text"
                onClick={editTaskPaneDesc}
              >
          ...
          <Button
            onClick={() => saveTaskPaneDesc()}
            className="!cursor-pointer !bg-gray-100 !text-black hover:!bg-dark2 hover:!text-white !my-2 !px-2 !py-4"
          >
            Save
          </Button>
        </Dialog.Content>
        ...
        <>
          <TextField.Root className="flex items-center ">
            <TextField.Input
              placeholder="Subtask"
              className="!p-2 "
              value={ssubTask.title}
              onChange={(e) =>
                ssetSubTask({ ...ssubTask, title: e.target.value })
              }
            ></TextField.Input>
            <TextField.Slot>
              <IconButton
        ...
        <>
          <div className="flex items-center  justify-between bg-white hover:bg-slate-100 border-2 border-transparent hover:border-slate-200 rounded-xl px-2 h-10">
            <div className=" flex-grow py-2 ">
              <h1 className="capitalize text-sm px-2 tracking-wider font-medium">
                {ssubTask.title}
              </h1>
            </div>
            <DropdownMenu.Root>
              <DropdownMenu.Trigger>
                <IconButton className=" !m-1 !bg-transparent hover:!bg-slate-200  !text-xs !text-black">
                  <MdMoreHoriz />
                </IconButton>
              </DropdownMenu.Trigger>
              <DropdownMenu.Content className="min-w-[10rem]">
                <DropdownMenu.Item
                  onClick={editSubTaskTitle}
                  className="z-10 !m-1  hover:!bg-neutral-600 hover:!text-white !text-black"
                >
                  Edit Title
                </DropdownMenu.Item>
                <DropdownMenu.Separator />
