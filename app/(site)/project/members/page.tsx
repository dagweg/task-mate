    ...
    <>
      <div className="flex justify-between items-center">
        <TopNav
          links={[
            {
              label: <IoPersonAddSharp />,
              link: "#",
              onClickCallback: () => toggleAddMemberDialog(),
              className:
                "add-member relative hover:!bg-gray-200 hover:!text-black rounded-sm !w-fit ",
            },
          ]}
                  ...
                  <tr key={i} className="border-b border-blue-gray-200">
                    ...
                    <DropdownMenu.Root>
                      <DropdownMenu.Trigger>
                        <IconButton className=" !m-1 !bg-gray-200 hover:!bg-neutral-300 !text-xl !text-black">
                          <IoIosMore />
                        </IconButton>
                      </DropdownMenu.Trigger>
                      <DropdownMenu.Content className="min-w-[10rem]">
                        <DropdownMenu.Item
                          onClick={() => handleRoleChange(mem)}
                          className=" !m-1  hover:!bg-gray-200  !text-black !flex  !flex-start !gap-3"
                        >
                          <RiAdminFill />
                        ...
                        <DropdownMenu.Item
                          ...
                          className=" !m-1  hover:!bg-red-500 hover:!text-white !text-black !flex  !flex-start !gap-3"
                        >
                          <FaRegTrashAlt />
      ...
      <Dialog.Root>
        ...
        <Dialog.Content>
          <div className="flex flex-col gap-10 justify-center">
            ...
            <Dialog.Close>
              <button
                className="bg-transparent border-2 p-2 hover:bg-dark2 text-black hover:text-white w-52 rounded-sm duration-75"
                onClick={addMembers}
              >
                Add
              </button>
        ...
        <RoleChangeDialog
          roleChangeDialogRef={roleChangeDialogRef}
          selected={selected}
          saving={savingRole}
          onSave={async (role: Role) => {
            if (!selected) return;
            setSavingRole(true);
...
function RoleChangeDialog({
  ...
  useEffect(() => {
    setRole(selected?.role || "TEAM_MEMBER");
  }, [selected]);
  return (
    <>
      <Dialog.Root>
          ...
          <div className="flex flex-col gap-10 justify-center">
            ...
            <Dialog.Close>
              <button
                ...
                className="bg-transparent border-2 p-2 hover:bg-dark2 text-black hover:text-white w-52 rounded-sm duration-75 disabled:opacity-60"
                onClick={() => onSave(role)}
              >
